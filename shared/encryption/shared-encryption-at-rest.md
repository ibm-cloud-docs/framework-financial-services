...
{{site.data.keyword.hscrypto}} and {{site.data.keyword.keymanagementserviceshort}} are both Financial Services Validated with the same level of security assurance. However, the strength of the security in {{site.data.keyword.keymanagementserviceshort}} as a multi-tenant service is lower than that provided by {{site.data.keyword.hscrypto}} as a single-tenant service with smartcards for the master key ceremony.
